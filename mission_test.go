...
func TestMissionGet(t *testing.T) {
	...
	_, err := client.Missions.Get(context.TODO(), 0)
	if err != nil {
...
func TestMissionListWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Missions.List(context.TODO(), earnapp.MissionListParams{
		Category: earnapp.String("category"),
		Status:   earnapp.MissionListParamsStatusActive,
	})
	if err != nil {
		var apierr *earnapp.Error
		if errors.As(err, &apierr) {
